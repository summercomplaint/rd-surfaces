...
export async function loadMeshFromFile(file) {
    ...
    // Weld duplicate-position vertices.
    // Three.js mergeVertices is unreliable here (OBJ with per-face normals
    // stores the same position multiple times; STL is fully non-indexed).
    // We do it ourselves: round each position to a grid and key into a Map.
    const posAttr  = geometry.attributes.position;
    const srcPos   = posAttr.array;                          // Float32Array
    const srcIdx   = geometry.index ? geometry.index.array : null;
    const nRef     = srcIdx ? srcIdx.length : srcPos.length / 3; // total vertex refs

    // Compute bbox diagonal for a scale-relative tolerance
    ...
    // 1e-5 of diagonal: small enough to not collapse real short edges,
    // big enough to catch floating-point near-duplicates from OBJ/STL export.
    ...
    // Remove degenerate faces — any face where two or more vertex indices
    // are the same (happens when nearby-but-distinct vertices were merged).
    let nGood = 0;
...
/**
 * Edge collapse pass: merges both endpoints of any edge shorter than minLen
 * into their midpoint, then removes degenerate faces.
 * Uses union-find so chains of collapses resolve correctly in one pass.
 */
function collapseShortEdges(pos, faces, minLen) {
    const work = pos.slice();           // copy — don't mutate caller's array
...
/**
 * One adaptive subdivision pass: splits only edges longer than `threshold`.
 * Faces with 0/1/2/3 edges split are re-triangulated without T-vertices.
 * Shared edges always get the same midpoint (via edgeKey), so the mesh
 * stays connected. Returns null if nothing was split.
 */
...
export function remeshForSimulation(positions, indices, minFaces = 12000) {
    ...
    // Target edge length for equilateral triangles covering the surface.
    const area    = surfaceArea(pos, faces);
    const targetH = Math.sqrt(4 * area / (Math.sqrt(3) * minFaces));
    const splitThr = targetH * 1.4;   // split edges > 1.4× target
    const collThr  = targetH * 0.6;   // collapse edges < 0.6× target

    console.log(`[remesh] input: ${faces.length} faces, targetH=${targetH.toFixed(4)}`);

    // Isotropic remeshing: alternate collapse (removes tiny faces) and
    // split (fills large faces) until convergence, up to 8 cycles.
    for (let iter = 0; iter < 8; iter++) {
        ...
        if (!sr && Math.abs(faces.length - nBefore) < 10) break; // converged
    }
...
export function computeCotangentLaplacian(positions, indices, opts = {}) {
    ...
    const edgeW = new Map(); // edgeKey -> weight
    ...
    // Floor each vertex area to 0.1% of the mean to prevent near-zero invArea
    // from blowing up the diffusion term for isolated or boundary vertices.
    let totalVA = 0;
    ...
    for (let i=0;i<n;i++) rowPtr[i+1]=rowPtr[i]+degree[i]+1; // +1 for diagonal
    const nnz = rowPtr[n];
    const colIdx = new Int32Array(nnz);
    const values = new Float64Array(nnz);
    const fill   = new Int32Array(n);

    for (let i=0;i<n;i++) { colIdx[rowPtr[i]]=i; fill[i]=1; } // diagonal placeholder
